...
describe('Integration Test', () => {
    ...
    const createBarcoin = (name, host, port, peers, removeData = true) => {
    ...
    const walletusername = 'test';
    let context = {};

    step('start server 1', () => {
        return createBarcoin(name1, 'localhost', 3001, [])
            .then((httpServer) => {
        ...
        return Promise.resolve()
            .then(() => {
                return supertest(context.httpServer1.app)
                    ...
                    .expect(200)
                    .expect((res) => {
                        assert.equal(res.body.balance, 90, `Expected balance of address '${context.address1}' to be '90'`);
                    });
            });
    });
    ...
    step('start server 2', () => {
        return createBarcoin(name2, 'localhost', 3002, [{ url: 'http://localhost:3001' }])
            .then((httpServer) => {
        ...
        return Promise.resolve()
            .then(() => {
                return supertest(context.httpServer1.app)
                    ...
                    .expect(200)
                    .expect((res) => {
                        assert.equal(res.body.balance, 79, `Expected balance of address '${context.address1}' to be '79'`);
                    });
            });
    });
